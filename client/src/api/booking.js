...
export const getBookings = async () => {
    try {
        ...
        const res = await fetch(
            "http://54.81.150.99:8080/bike-me-now/api/reservas",
            {
                headers: {
...
export const getBookingsByUserId = async (id) => {
    try {
        const { token } = JSON.parse(localStorage.getItem("accessToken"));
        const res = await fetch(
            `http://54.81.150.99:8080/bike-me-now/api/reservas/usuarios/${id}`,
            {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
...
export const postRating = async (rating) => {
    try {
        const { token } = JSON.parse(localStorage.getItem("accessToken"));
        const res = await fetch(
            "http://54.81.150.99:8080/bike-me-now/api/valoraciones",
            {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${token}`,
...
export const deleteBooking = async (id) => {
    ...
    try {
        const res = await fetch(
            `http://54.81.150.99:8080/bike-me-now/api/reservas/${id}`,
            {
                method: "DELETE",
                headers: {
                    // "Content-Type": "application/json",
